...
class SerialPort(AbstractSerial):
    ...
    def __init__(self) -> None:
        ...
        self.isopen = False
        self.receivedData = []
        self._data: bool = False             # una bandera para saber si hay datos 
        self._bufferFull: bool = False       # una bandera para saber si el buffer estálleno
            ...
            try:
                if self.isopen:
                    ...
                    if self.serialport.in_waiting() > 0:
                        self._data = True #TODO: find the point where put it False again (Observer, method already created)
                        # start_time = time.time() 
                        
                        self._state = True #COMEBACK:   what do i do with this flag? seria el mismo de data

                        # print(f'<SerialPorti><readLine>: start_time: {start_time}')
                        print(f'<SerialPorti><readLine>: Hay dato en puerto')
                        
                        receivedBit = self.serialport.read() #FIXME: No deja que el bucle se ejecute permanentemente, se queda bloqueado
                        
                        # self.receivedData.append(receivedBit.decode("utf-8"))
                        self.receivedData.append(receivedBit)
                        if len(self.receivedData) > 100:
                            print(f'<SerialPorti><readLine>: Buffer is bigger then 100 {len(self.receivedData)}')
                            self.notify()
                            self.bufferFull_status = True
                            self.receivedData.clear()
                        else:
                            # print(f'<SerialPorti><readLine>: time- {time.time() - start_time}s')
                            print(f'<SerialPorti><readLine>: self._data: {self._data}')
                    else:
                        print(f'<SerialPorti><readLine>: No data')
                        
                    """ if (time.time() - start_time > 5) and self._data:
                        print(f'<SerialPorti><readLine>: timeout - {start_time - time.time()}s')
                        self.notify()
                        self.receivedData.clear() """
